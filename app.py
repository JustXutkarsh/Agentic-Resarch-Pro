...
# ==============================================================================
# View State: Screen 3 (Results Workspace) vs Screen 1 (Editorial Home)
# ==============================================================================
...
if active_result is not None:
    ...
    with col_nav_left:
        st.markdown(
            f"""<div style="font-size:13px; color:#6E6E73; font-weight:600; padding-top:6px;">
            ✦ Viewing Research Dossier: <span style="color:#1C1C1E; font-weight:750;">{active_result.topic}</span>
            </div>""",
...
<div style="font-size:11px; font-weight:800; color:#6E6E73; text-transform:uppercase; letter-spacing:1px;">
Select Research Depth
        ...
        with col_q:
            ...
            cls_q = "depth-col-selected" if is_q else "depth-col"
            st.markdown(f'<div class="{cls_q}">' + render_single_depth_card("QUICK", is_selected=is_q), unsafe_allow_html=True)
        ...
        with col_s:
            ...
            cls_s = "depth-col-selected" if is_s else "depth-col"
            st.markdown(f'<div class="{cls_s}">' + render_single_depth_card("STANDARD", is_selected=is_s), unsafe_allow_html=True)
            if st.button("✓ Active Mode" if is_s else "Select Standard (Recommended)", key="btn_depth_s", use_container_width=True):
                st.session_state["depth_choice"] = "STANDARD"
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        with col_d:
            ...
            cls_d = "depth-col-selected" if is_d else "depth-col"
            st.markdown(f'<div class="{cls_d}">' + render_single_depth_card("DEEP", is_selected=is_d), unsafe_allow_html=True)
        ...
        st.markdown(
            """<div style="text-align:center; margin-top:22px; margin-bottom:10px; font-size:11px; font-weight:800; color:#6E6E73; text-transform:uppercase; letter-spacing:1px;">
            Curated Questions
            </div>""",
            unsafe_allow_html=True
        )
    ...
    if launch_clicked:
        ...
        # Telemetry and Visual Containers (Clean 7-stage flow + metrics + live terminal)
        ...
        def live_progress_handler(step_name: str, pct: float, details: str):
            ...
            elapsed_int = int(curr_elapsed)
            timestamp = f"+{elapsed_int//60:02d}:{elapsed_int%60:02d}"
            ...
            terminal_container.markdown(
                render_live_activity_terminal(event_logs, current_message=friendly_text, elapsed_seconds=curr_elapsed),
                unsafe_allow_html=True,
            )
        ...
        try:
            ...
            terminal_container.markdown(
                render_live_activity_terminal([], current_message="Understanding question...", elapsed_seconds=0.0),
                unsafe_allow_html=True,
            )
            ...
            terminal_container.markdown(
                render_live_activity_terminal(event_logs, current_message="Research completed & dossier verified.", elapsed_seconds=time.time() - start_time),
                unsafe_allow_html=True,
            )
